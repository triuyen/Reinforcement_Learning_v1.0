...
impl<E: ModelFreeEnv> MonteCarloAgent<E> {
    ...
    pub fn new(mut env: E, episodes: usize, gamma: f32, epsilon: f32) -> Self {
    ...
    pub fn epsilon_greedy_policy(&mut self, state: usize) -> usize {
        let mut rng = rand::thread_rng();
        let available_actions = self.env.available_actions();
    
        if available_actions.is_empty() {
            panic!("No available actions for state {}!", state);
        }
        
        // Ensure q_value and returns are properly sized (out of bound error)
        ...
        if rng.gen::<f32>() < self.epsilon {
            ...
            *available_actions.choose(&mut rng).unwrap()
            ...
            // Select the best action based on q_values
            let best_action = available_actions
                .iter()
                .max_by(|&&a, &&b| {
                    if a >= self.q_value[state].len() || b >= self.q_value[state].len() {
                        eprintln!(
                            "Invalid action index: action_a={}, action_b={} (q_value[{}].len()={})",
                            a, b, state, self.q_value[state].len()
                        );
                        return std::cmp::Ordering::Equal;
                    }
                    self.q_value[state][a].partial_cmp(&self.q_value[state][b]).unwrap()
                })
                .copied(); // Handle potential Option
    
            best_action.unwrap_or_else(|| *available_actions.choose(&mut rng).unwrap())
        }
    }
